...
def recognize(prop):
    ...
    if euler_number == -1:  # два бублика 8 или B
        if has_vline(prop.image):
            return "B"
        else:
            return "8"
    elif euler_number == 0:  # один бублик A 0 P D
        if has_vline(prop.image):  # P D
            ...
            if np.isclose(x, y, 0.04):
                ...
                return "A"
    elif euler_number == 1:  # нет бубликов :\ ) 1 W X * - /
...
if __name__ == '__main__':
    ...
    print(np.max(labeled))  # type: ignore
    ...
    for prop in props:
        ...
        result[symbol] += 1
    print(result)
    print((1 - result.get("_", 0) / np.max(labeled)) * 100)  # type: ignore

    plt.imshow(labeled)  # type: ignore
    plt.show()
